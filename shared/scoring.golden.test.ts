...
import {
  computeResult, roundScore, W_CUISINE, W_DISTANCE, W_PRICE, W_RATING,
} from './scoring';
import {
  ALGORITHM_VERSION, type Participant, type Prefs, type Tiebreak,
} from './types';
...
const baseRestaurant: Omit<DemoRestaurantInput, 'id' | 'name'> = {
  cuisines: ['Italian'], priceTier: 2, rating: 4.5, distanceKm: 1,
...
describe(`golden ranking fixtures ${ALGORITHM_VERSION}`, () => {
  it('matches every versioned expected result and reconciles audit components', () => {
    ...
    for (const scenario of fixture.cases) {
      ...
      const restaurants = normalizeDemoRestaurants(scenario.restaurants.map((input) => ({
        ...baseRestaurant, ...input, name: input.id,
      } as DemoRestaurantInput)));
      const rerun = scenario as typeof scenario & {
        excludedIds?: string[]; round?: number; previousWinners?: string[];
      };
      ...
      expect(result.scoringSheet.filter((row) => !row.eliminated).map((row) => row.restaurantId), scenario.id)
        .toEqual(scenario.expected.order);
      expect(result.tiebreak, scenario.id).toBe(scenario.expected.tiebreak as Tiebreak);
      expect(Object.fromEntries(result.scoringSheet.map((row) => [row.restaurantId, row.total])), scenario.id)
        .toEqual(scenario.expected.totals);
