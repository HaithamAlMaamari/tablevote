...
import { inviteSnapshot, snapshot, type Participant, type Prefs, type Restaurant, type Session } from './types';
import { normalizeDemoRestaurants } from './catalog';

const restaurant = (id: string, cuisine: Restaurant['cuisines'][number]): Restaurant => normalizeDemoRestaurants([{
  id, name: id, cuisines: [cuisine], priceTier: 2, rating: 4.5, distanceKm: 1,
  lat: 23.5, lng: 58.3, address: 'Muscat', dietaryOptions: [], openNow: true,
}])[0];
const prefs = (cuisines: Prefs['cuisines']): Prefs => ({
  cuisines, budget: 2, maxDistanceKm: 3, dietary: [],
});

describe('participant projections', () => {
  it('keeps duplicate display names distinct and exposes only the viewer fit', () => {
    const participants: Participant[] = [
      { id: 'p1', token: 't1', nickname: 'Same', color: 0, prefs: prefs({ Italian: 'like' }), submittedAt: 1, isHost: true },
      { id: 'p2', token: 't2', nickname: 'Same', color: 1, prefs: prefs({ Japanese: 'like' }), submittedAt: 1, isHost: false },
    ];
    ...
    const session: Session = {
      id: 'session', code: 'ABCDE', hostToken: 'host', participants, phase: 'revealed',
      result, excludedIds: [], rerunsUsed: 0, allowReruns: true, createdAt: 1,
      center: { lat: 23.5, lng: 58.3 }, areaLabel: 'Qurum', radiusKm: 3,
    };

    const first = snapshot(session, participants[0]);
    const second = snapshot(session, participants[1]);
    ...
    expect(inviteSnapshot(session)).toEqual({
      code: 'ABCDE', areaLabel: 'Qurum', expiresAt: 1 + 24 * 60 * 60 * 1000, joinable: false,
    });
    session.shareHostNickname = true;
    expect(inviteSnapshot(session)).toEqual({
      code: 'ABCDE', areaLabel: 'Qurum', expiresAt: 1 + 24 * 60 * 60 * 1000,
      joinable: false, hostNickname: 'Same',
    });
  });
});
