...
| Oversized traffic           | Express/Socket.IO payload limits plus global, trusted-address, socket, session, and malformed-transport quotas              | Distributed traffic can exhaust bounded global capacity and deny service                                                    |
| Memory exhaustion           | Engine.IO global/address ceilings and unattached deadlines plus caps on application state and quota keys                    | Limits are per process and need operational sizing and distributed controls                                                 |
