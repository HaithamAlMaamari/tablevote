...
export function SessionIssueAlert({ issue }: { issue: SessionIssue }) {
  ...
  return (
    <div role="alert" className="mt-5 border-l-[6px] border-signal bg-signal-tint p-4 text-left">
      <p className="font-semibold text-ink">{content.title}</p>
      <p className="mt-1 text-[14px] leading-relaxed text-ink-muted">{content.detail}</p>
    </div>
  );
}
  ...
  return (
    <ScreenShell>
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-6 text-center">
        <h1 ref={headingRef} tabIndex={-1} className="font-display text-[26px] font-semibold text-ink">
          {content.title}
        </h1>
        <p className="mt-2 max-w-sm text-[15px] leading-relaxed text-ink-muted">{content.detail}</p>
        <Btn
          className="mt-6 px-6"
          onClick={() => {
