...
export const createSession = (options: CreateOpts): CreatedSession => {
  const result = defaultStore.create(options);
  if (!result.ok) throw new Error(result.failure.message);
  return result.value;
};
export const getByIdOrCode = (idOrCode: string) => defaultStore.get(idOrCode);
export const getTerminalReason = (idOrCode: string) => defaultStore.terminalReason(idOrCode);
export const onSessionExpired = (listener: (session: Session) => void) => defaultStore.onExpired(listener);
export const findParticipant = (session: Session, participantToken: string) =>
  defaultStore.findParticipant(session, participantToken);
export const joinSession = (session: Session, nickname: string, color: number) =>
  (() => {
    const result = defaultStore.join(session, nickname, color);
    return result.ok
      ? { participantToken: result.value.participantToken, participantId: result.value.participantId }
      : { error: result.failure.message };
  })();
export const submitPrefs = (session: Session, participantToken: string, prefs: Prefs) =>
  defaultStore.submit(session, participantToken, prefs).ok;
export const leaveSession = (session: Session, participantToken: string) =>
  (() => {
    const result = defaultStore.leave(session, participantToken);
    return result.ok
      ? { ok: true as const, participantId: result.value.participantId }
      : { ok: false as const, error: result.failure.message };
  })();
export const removeParticipant = (session: Session, hostToken: string, participantId: string) =>
  (() => {
    const result = defaultStore.remove(session, hostToken, participantId);
    return result.ok ? { ok: true as const } : { ok: false as const, error: result.failure.message };
  })();
export const reveal = (session: Session, hostToken: string, onLocked?: () => void) =>
  (() => {
    const result = defaultStore.reveal(session, hostToken, onLocked);
    return result.ok
      ? { ok: true as const, changed: result.value.changed }
      : { ok: false as const, error: result.failure.message };
  })();
export const rerun = (session: Session, hostToken: string, onLocked?: () => void) =>
  (() => {
    const result = defaultStore.rerun(session, hostToken, onLocked);
    return result.ok ? { ok: true as const } : { ok: false as const, error: result.failure.message };
  })();
export const endSession = (idOrCode: string, hostToken: string) => defaultStore.end(idOrCode, hostToken).ok;
export const getStoreResourceCounts = () => defaultStore.resourceCounts();
