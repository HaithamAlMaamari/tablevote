...
describe('terminal socket lifecycle matrix', () => {
  ...
  it('preserves ended and expired reasons for every fresh socket mutation', async () => {
    ...
    expect((await request(app).post(`/api/sessions/${ended.sessionId}/end`).send({
      hostToken: ended.hostToken,
    })).status).toBe(200);
    await expectTerminalMutations(ended, 'Session ended', 'ended');

    const expired = await createAndAttach();
    getByIdOrCode(expired.sessionId)!.createdAt = Date.now() - SESSION_TTL_MS;
  ...
  it('reattaches replayed create and join mutations to private rooms', async () => {
    const createInput = {
      areaLabel: 'Qurum', center: { lat: 23.588, lng: 58.3829 }, radiusKm: 10,
      nickname: 'Replay Host', color: 0, allowReruns: true,
    ...
    await request(app).post('/api/sessions/join').send({
      code: created.body.code, nickname: 'First', color: 1,
    });
    expect((await hostUpdate).participants).toHaveLength(2);

    const joinInput = {
      code: created.body.code, nickname: 'Replay Guest', color: 2,
    ...
    await request(app).post('/api/sessions/join').send({
      code: created.body.code, nickname: 'Last', color: 3,
    });
  ...
  it('keeps revealed sessions unchanged across rejected REST and socket mutations', async () => {
    ...
    const guest = await request(app).post('/api/sessions/join').send({
      code: session.code, nickname: 'Guest', color: 1,
    });
    for (const token of [session.participantToken, guest.body.participantToken]) {
      expect((await request(app).post(`/api/sessions/${session.sessionId}/submit`).send({ token, prefs })).status).toBe(200);
    }
    expect((await request(app).post(`/api/sessions/${session.sessionId}/reveal`).send({
      hostToken: session.hostToken,
    })).status).toBe(200);
    const before = JSON.stringify(getByIdOrCode(session.sessionId));
    ...
    expect(JSON.stringify(getByIdOrCode(session.sessionId))).toBe(before);
    expect((await request(app).post(`/api/sessions/${session.sessionId}/end`).send({
      hostToken: session.hostToken,
    })).status).toBe(200);
  });
  ...
  it('broadcasts server-backed participant presence on attach and disconnect', async () => {
    const session = await createAndAttach();
    const guest = await request(app).post('/api/sessions/join').send({
      code: session.code, nickname: 'Presence Guest', color: 1,
    });
    ...
    try {
      const onlineState = new Promise<{ participants: { id: string; online: boolean }[] }>((resolve) => {
        socket.on('state', (state) => {
          if (state.participants.find((participant: { id: string }) => participant.id === guest.body.participantId)?.online) {
            resolve(state);
          }
        });
      });
      await new Promise((resolve) => guestSocket.emit('attach', {
        sessionId: session.sessionId, token: guest.body.participantToken,
      }, resolve));
      ...
      const offlineState = new Promise<{ participants: { id: string; online: boolean }[] }>((resolve) => {
        socket.on('state', (state) => {
          if (state.participants.find((participant: { id: string }) => participant.id === guest.body.participantId)?.online === false) {
            resolve(state);
          }
        });
      });
      ...
      expect(offline.participants.find((participant) => participant.id === guest.body.participantId)?.online).toBe(false);
  ...
  it('keeps blocked-no-match sessions unchanged across rejected socket mutations', async () => {
    const session = await createAndAttach();
    const guest = await request(app).post('/api/sessions/join').send({
      code: session.code, nickname: 'Strict Guest', color: 1,
    });
    const dietary = ['vegetarian', 'vegan', 'halal', 'kosher', 'gluten-free']
      .map((type) => ({ type, strict: true }));
    const strictPrefs = { ...prefs, dietary };
    for (const token of [session.participantToken, guest.body.participantToken]) {
      expect((await request(app).post(`/api/sessions/${session.sessionId}/submit`).send({
        token, prefs: strictPrefs,
      })).status).toBe(200);
    }
    expect((await request(app).post(`/api/sessions/${session.sessionId}/reveal`).send({
      hostToken: session.hostToken,
    })).status).toBe(200);
    expect((await request(app).post(`/api/sessions/${session.sessionId}/rerun`).send({
      hostToken: session.hostToken,
    })).status).toBe(200);
    expect(getByIdOrCode(session.sessionId)?.phase).toBe('blocked-no-match');
    const before = JSON.stringify(getByIdOrCode(session.sessionId));
